...
export default function ResumePreview({ resumeData, selectedTemplate }: ResumePreviewProps) {
  const renderSpecificTemplate = () => {
    if (selectedTemplate && selectedTemplate.component) {
      // Use React.createElement for dynamically rendering components
      // This is necessary when the component type comes from a variable
      return React.createElement(selectedTemplate.component, { resumeData });
    }
    return null;
  };

  const specificTemplateContent = renderSpecificTemplate();
  ...
  return (
    <SectionCard title="Live Preview" icon={<Eye className="h-5 w-5" />} className="sticky top-6">
      ...
      {specificTemplateContent ? (
        ...
        // Fallback to generic preview if no specific template component
        <div className="bg-muted/30 p-6 rounded-lg min-h-[600px] prose prose-sm max-w-none">
          {selectedTemplate && (
            <div className="mb-4 p-2 bg-accent/20 rounded text-center">
              ...
              <p className="text-xs text-muted-foreground">(Generic Preview)</p>
            </div>
          )}
